            ...
            <el-form :model="form">
                <el-form-item label="类别" :label-width="formLabelWidth">
                    <el-select v-model="form.category_id" placeholder="请选择类别">
                        <el-option v-for="cat in catInfoData" :key="cat.category_id" :label="cat.cat_name" :value="cat.category_id" />
                    </el-select>
                </el-form-item>
...
export default {
    ...
    methods: {
        ...
        getPageData(num, size) {
            this.$http.get('/question/v1/questionInfo', { params: { pageNum: num, pageSize: size } })
                .then((response) => {
        ...
        addQuestion() {
            this.$http.post('/question/v1/questionS', this.form).then((response) => {
                if (response.data == 1) {
        ...
        openDetailDialog(questionId) {
            // 获取并显示详情
            this.$http.get(`/question/v1/question/${questionId}`).then((response) => {
        ...
        singleDelete(questionId) {
            ElMessageBox.confirm('确定删除这条记录吗?', '删除提示', {
                ...
                this.$http.delete(`/question/v1/questionS/${questionId}`).then((response) => {
                    if (response.data == 1) {
        ...
        multipleDelete() {
            if (this.multipleSelection.length > 0) {
                ElMessageBox.confirm('是否删除选中的所有数据?', '批量删除提示', {
                    ...
                    this.$http.post('/question/v1/questionDelBatch', this.multipleSelection).then((response) => {
                        if (response.data == 1) {
        ...
        updateQuestion() {
            this.$http.put(`/question/v1/questionS/${this.form.question_id}`, this.form).then((response) => {
                if (response.data == 1) {
